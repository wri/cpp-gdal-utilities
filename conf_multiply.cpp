...
{
//passing arguments
if (argc != 4){cout << "Use <program name> <input name> <output name> <multiply_val>" << endl; return 1;}
string in_name=argv[1];
string out_name=argv[2];
uint16_t day_offset=atoi(argv[3]);
...
uint16_t out_data[xsize];

for(y=0; y<ysize; y++) {
INBAND->RasterIO(GF_Read, 0, y, xsize, 1, in_data, xsize, 1, GDT_UInt16, 0, 0); 
for(x=0; x<xsize; x++) {
if (in_data[x]>0){out_data[x]=in_data[x]*day_offset ;}
else {out_data[x]=0;}
}
